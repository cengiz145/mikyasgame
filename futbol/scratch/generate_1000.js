...
for (let i = 0; i < 1000; i++) {
    ...
    const id = `${fName.toLowerCase()}_${lName.toLowerCase()}_${i}`;
...
let fileContent = `// js/data_1000.js\n// Yapay Zeka tarafindan otomatik uretilmis 1000 Serbest Oyuncu (Free Agent Havuzu)\n\nwindow.leagueData = window.leagueData || { teams: [], players: [] };\n\n`;
fileContent += `const freeAgentPool = ${JSON.stringify(players, null, 4)};\n\n`;
fileContent += `freeAgentPool.forEach(p => window.leagueData.players.push(p));\n`;

fs.writeFileSync('js/data_1000.js', fileContent, 'utf-8');
console.log('1000 oyuncu basariyla js/data_1000.js dosyasina yazildi!');
