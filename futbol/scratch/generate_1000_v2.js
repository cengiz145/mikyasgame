...
for (let i = 0; i < 1000; i++) {
    ...
    // Contract years: free_agent generally has 1. But let's set 1-3.
    const contractYears = Math.floor(Math.random() * 3) + 1;
    
    const id = `${fName.toLowerCase()}_${lName.toLowerCase()}_v2_${i}`;
...
let fileContent = `// js/data_1000_v2.js\n// Yapay Zeka tarafindan otomatik uretilmis 1000 Ekstra Serbest Oyuncu (Free Agent Havuzu)\n\nwindow.leagueData = window.leagueData || { teams: [], players: [] };\n\n`;
fileContent += `const freeAgentPoolV2 = ${JSON.stringify(players, null, 4)};\n\n`;
fileContent += `freeAgentPoolV2.forEach(p => window.leagueData.players.push(p));\n`;

fs.writeFileSync('js/data_1000_v2.js', fileContent, 'utf-8');
console.log('Ekstra 1000 oyuncu basariyla js/data_1000_v2.js dosyasina yazildi!');
