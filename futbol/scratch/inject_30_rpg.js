...
// We need to properly format the injected array objects as raw string
let stringifiedEvents = newEvents.map(e => {
    let opts = e.options.map(o => {
        let act = o.effect.customAction ? \`customAction: \${o.effect.customAction.toString()}\` : '';
        let effs = [];
...
console.log('30 New RPG Events Injected Successfully!');
