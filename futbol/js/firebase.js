...
try {
    if (typeof firebase !== 'undefined') {
        ...
        console.log("🔥 Firebase Cloud Save aktif!");
    } else {
        throw new Error("Firebase kütüphanesi yüklenemedi.");
    }
} catch (error) {
    console.warn("☁️ [ÇEVRİMDİŞİ MOD AKTİF] Veritabanı bağlantısı yok veya internet koptu:", error);
    window.db = null; // Sistem çevrimdışı oynanış için db'yi nazikçe atlar
}

// Global User ID tanımla (Cloud Save için her tarayıcıya/kullanıcıya özel ID)
let footballUserId = localStorage.getItem('footballUserId');
if (!footballUserId) {
    footballUserId = 'user_' + Math.random().toString(36).substr(2, 9);
    localStorage.setItem('footballUserId', footballUserId);
}
window.userId = footballUserId;
